...
if section == "Current Stats / KPI":
    ...
    def load_formality_data():
        data = supabase.table("enhanced_treated_restaurants").select("*").execute().data
        return pd.DataFrame(data)

    df = load_formality_data()

    if df.empty or "formality_old" not in df or "formality_new" not in df:
        st.warning("❌ No formality data available.")
        st.stop()

    df["formality_old"] = df["formality_old"].fillna("?").str.strip().str.lower()
    df["formality_new"] = df["formality_new"].fillna("?").str.strip().str.lower()

    # ------------------------ CHART 1 ------------------------
    st.markdown("### 📊 Old vs New Formality Status")

    df_old = df[~df["formality_old"].isin(["?"])]
    df_new = df[~df["formality_new"].isin(["?", "", "inactive", "not filing inactive"])]

    old_counts = df_old["formality_old"].value_counts().sort_index()
    new_counts = df_new["formality_new"].value_counts().sort_index()

    labels = sorted(set(old_counts.index).union(set(new_counts.index)))
    old_counts = old_counts.reindex(labels, fill_value=0)
    new_counts = new_counts.reindex(labels, fill_value=0)

    x = range(len(labels))
    width = 0.35

    fig1, ax1 = plt.subplots(figsize=(10, 5))
    bars_old = ax1.bar([i - width/2 for i in x], old_counts, width, label="Old Formality", color="#1f77b4")
    bars_new = ax1.bar([i + width/2 for i in x], new_counts, width, label="New Formality", color="#ff7f0e")
    ...
    ax1.set_title("Old vs New Formality Status")
    ax1.legend()
    st.pyplot(fig1)

    # ------------------------ CHART 2 ------------------------
    st.markdown("### 🔁 Transition Paths: Old → New Formality")

    valid_old = ["filed", "registered", "unregistered"]
    valid_new = ["filed", "registered", "unregistered"]

    filtered = df[df["formality_old"].isin(valid_old) & df["formality_new"].isin(valid_new)].copy()
    total_old = filtered["formality_old"].value_counts().to_dict()

    transitions = (
        filtered.groupby(["formality_old", "formality_new"])
        .size().reset_index(name="count")
    )
    pivot = transitions.pivot(index="formality_old", columns="formality_new", values="count").fillna(0)

    fig2, ax2 = plt.subplots(figsize=(10, 5))
    x2 = range(len(pivot.index))
    width2 = 0.8 / len(pivot.columns)
    colors = {"filed": "#1f77b4", "registered": "#ff7f0e", "unregistered": "#2ca02c"}

    for i, col in enumerate(pivot.columns):
        bars = ax2.bar(
            [xi + i * width2 for xi in x2], pivot[col].values, width2,
            label=col.title(), color=colors[col]
        )
        for bar in bars:
            height = bar.get_height()
            if height > 0:
                ax2.annotate(f"{int(height)}", (bar.get_x() + bar.get_width() / 2, height),
                            textcoords="offset points", xytext=(0, 3), ha='center', fontsize=8)

    # Add total count above each old category
    for idx, label in enumerate(pivot.index):
        ax2.text(idx, ax2.get_ylim()[1]*0.95, f"n={total_old.get(label, 0)}", 
                ha='center', fontsize=9, color="gray")

    ax2.set_xticks([xi + width2 * (len(pivot.columns)/2 - 0.5) for xi in x2])
    ax2.set_xticklabels(pivot.index, rotation=45, ha="right")
    ax2.set_ylabel("Number of Restaurants")
    ax2.set_title("Old → New Formality Transitions (with Totals)")
    ax2.legend(title="New Status", bbox_to_anchor=(1.05, 1), loc="upper left")
    st.pyplot(fig2)
